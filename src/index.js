...
async function loadDataAndCreateCharts() {
    ...
    Object.entries(groups).forEach(([group, participants]) => {
        ...
        container.selectAll(".participant-section").remove(); // Clear previous sections
        // adding 5 div to the container for each participant
...
function createGlucoseLineChart(container, data, groupColor, yScale) {
    ...
    let width = container.node().getBoundingClientRect().width - 10;
